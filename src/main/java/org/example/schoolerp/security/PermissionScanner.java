...
/**
 * PermissionScanner scans the base package for hasAuthority annotations in order to collect
 * permissions
 */
@Component
@Slf4j
public class PermissionScanner {
  ...
  public PermissionScanner(
      @Value("${school-erp.security.permission-scan-package}") String basePackage) {
  ...
  public Set<String> scan() {
    ...
    for (BeanDefinition definition : scanner.findCandidateComponents(basePackage)) {
      ...
      try {
        Class<?> clazz =
            ClassUtils.forName(definition.getBeanClassName(), ClassUtils.getDefaultClassLoader());

        scanClass(clazz, permissions);

      } catch (ClassNotFoundException e) {
        throw new IllegalStateException(
            "Could not scan class: " + definition.getBeanClassName(), e);
      }
    }
  ...
  private void scanClass(Class<?> clazz, Set<String> permissions) {
    PreAuthorize classAnnotation = AnnotationUtils.findAnnotation(clazz, PreAuthorize.class);
    ...
    for (var method : clazz.getDeclaredMethods()) {
      PreAuthorize methodAnnotation = AnnotationUtils.findAnnotation(method, PreAuthorize.class);
  ...
  private void extractPermissions(PreAuthorize annotation, Set<String> permissions) {
    Matcher expressionMatcher = AUTHORITY_EXPRESSION.matcher(annotation.value());

    while (expressionMatcher.find()) {

      String arguments = expressionMatcher.group(1);

      Matcher authorityMatcher = AUTHORITY_NAME.matcher(arguments);
