...
public class TenantFixtures {
  ...
  /**
   * Creates an loginable user. Caller is responsible for running this inside asTenant(...)/newTx()
   * with TenantContext already set. (org creation is tenant-native) followed by the correct tenant.
   */
  ...
  /**
   * Deletes the User/AuthAccount rows that belong to ONE tenant(Set by the caller). Must be called
   * witha fresh REQUIRES_NEW transaction wrapping it
   */
